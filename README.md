...
| `FULL SCREEN (WIP)` | on / off | **work in progress.** Takes the shape of the device and shows **several boxes at once** — as many 5×4 panels as fit, across then down, each with its own name and its own wallpaper. UP from a panel's top row lands on its name, and A there opens the BOX MENU for *that* box. Off the last panel the page scrolls. Overrides `GRID`; works on Gold too |
